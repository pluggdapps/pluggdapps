...
Once interfaces are defined, they are implemented by one or more plugin classes.
A plugin class can implement more than one interface. When a plugin implements
an interface, it should define methods and attributes specified by that 
interface. Plugin developers must stick to the semantic meaning of the
interface(s) that the plugin is going to implement.

**Every plugin is nothing but a dictionary of configuration settings**. It is 
the responsibility of the platform to gather configuration information from
different sources like ini-file(s) and database, initialize the plugin
...
into the environment during platform startup. Pluggdapps packages must, in
turn, load interfaces and plugins defined by the package. Finally, when all 
